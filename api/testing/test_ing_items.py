...
def test_get_item_by_uid():
    ...
    response_user = httpx.get(
        f"{BASE_URL}/items/P000001", headers=user_headers)
...
def test_get_items_for_item_line():
    item_line_id = 58
    response = httpx.get(
        f"{BASE_URL}/items/item_line/{item_line_id}", headers=admin_headers)
    assert response.status_code == 200

    item_line_id_user = 11
    response_user = httpx.get(
        f"{BASE_URL}/items/item_line/{item_line_id_user}", headers=user_headers)
...
def test_get_items_for_item_group():
    item_group_id = 50
    response = httpx.get(
        f"{BASE_URL}/items/item_group/{item_group_id}", headers=admin_headers)
    assert response.status_code == 200

    item_group_id_user = 73
    response_user = httpx.get(
        f"{BASE_URL}/items/item_group/{item_group_id_user}", headers=user_headers)
...
def test_get_items_for_item_type():
    item_type_id = 63
    response = httpx.get(
        f"{BASE_URL}/items/item_type/{item_type_id}", headers=admin_headers)
    assert response.status_code == 200  # returns error 404

    item_type_id_user = 14
    response_user = httpx.get(
        f"{BASE_URL}/items/item_type/{item_type_id_user}", headers=user_headers)
...
def test_get_items_for_supplier():
    supplier_id = 69
    response = httpx.get(
        f"{BASE_URL}/items/supplier/{supplier_id}", headers=admin_headers)
    assert response.status_code == 200  # retrun code 404 notfound

    supplier_id = 34
    response = httpx.get(
        f"{BASE_URL}/items/supplier/{supplier_id}", headers=user_headers)
...
def test_add_items():
    ...
    response = httpx.post(f"{BASE_URL}/items",
                          headers=admin_headers, json=data,)
    assert response.status_code == 201

    # this assumes GET is working correctly
    get_response = httpx.get(
        f"{BASE_URL}/items/P011722", headers=admin_headers)
    ...
    response_user = httpx.post(
        f"{BASE_URL}/items", headers=user_headers, json=data_user)
...
def test_update_items():
    ...
    response = httpx.put(f"{BASE_URL}/items/P000002",
                         headers=admin_headers, json=data,)
    assert response.status_code == 200

    get_response_updated = httpx.get(
        f"{BASE_URL}/items/P011723", headers=admin_headers)
    ...
    response_user = httpx.put(
        f"{BASE_URL}/items/P011722", headers=user_headers, json=data_user)
...
def test_delete_item():
    ...
    response_user = httpx.delete(
        f"{BASE_URL}/items/P011723", headers=user_headers)
    assert response_user.status_code == 403
