...
def test_add_item_types():
    ...
    get_response = httpx.get(f"{BASE_URL}/item_types/101", headers=admin_headers)
...
def test_update_item_types():
    ...
    response = httpx.put(f"{BASE_URL}/item_types/0", headers=admin_headers, json=data,)
    assert response.status_code == 200

    # this assumes GET is working correctly
    get_response = httpx.get(f"{BASE_URL}/item_types/105", headers=admin_headers)
    assert get_response.status_code == 200  # Ensure the GET request is successful
    fetched_data = get_response.json()
